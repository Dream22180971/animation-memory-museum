...
type SongCardsProps = {
  /** 预览模式：首页只放 6 盘磁带，正片在 /songs */
  preview?: boolean;
};

export default function SongCards({ preview = false }: SongCardsProps) {
  const cassettes = preview ? animationRecords.slice(0, 6) : animationRecords;
  return (
    <section id="songs" className="scroll-mt-24 px-5 py-12 sm:px-10 lg:px-14">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        ...
        {preview ? (
          <Link href="/songs" className="retro-button retro-button-ghost text-xs">
            全部 {animationRecords.length} 盘卡带
            <ArrowRight size={14} />
          </Link>
        ) : null}
      </div>
